...
#Link to github explanation is in beginning of lecture from 10/28
...
vaxrates <- read_csv("https://data.cdc.gov/api/views/fhky-rtsk/rows.csv?accessType=DOWNLOAD")

glimpse(WorldCupMatches)
WorldCupMatches %>% select(-2, -3, -4, -11, -15, -16) %>% select(-11, -12) -> worldcup
head(worldcup)
glimpse(GDP)
...
worldcup %>% mutate(Home_Team_Goals= Half.time.Home.Goals + Home.Team.Goals) %>% mutate(Away_Team_Goals= Half.time.Away.Goals + Away.Team.Goals) %>% mutate(who_wins= ifelse(Home_Team_Goals > Away_Team_Goals, "Home", "Away"))
# Filter
proj_join %>% filter(Country == "bra") %>% filter( Year == 2010)
glimpse(proj_join)
#In 2010, Brazil had a GDP growth of 7.534 (which is a lot) and they won 3 matches in the WC
# Select
proj_join %>% select(1:3,8, 9, 10, everything()) -> proj_join
proj_join %>% select(-11)  -> proj_join
# Stringr
proj_join %>% str_match_all("19[0-9]+")
# Summarize (5)
...
#Turkey which had the highest average for goals as a home team had a standard deviation of 4.24 indicating that they varied signficantly over the years in performance
...
#Let's you see the change in GDP Growth over the years
...
- Use all six core `dplyr` functions in the service of generating summary tables/statistics (14 pts)
    ...
    - Use at least one `stringr` function with something to do with regex
